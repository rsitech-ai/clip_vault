...
- Modify: `<local-codex-config>/PLANS.md` progress log outside the repository.
...
- Complete gates passed: final exact run of 142 Swift tests in 17 suites, 4 Rust tests, strict release warnings, rustfmt, strict Clippy, and syntax checks for 17 shell scripts. Focused suites passed 12 generated-prompt persistence, 27 folder/migration, 45 prompt-workflow, and 9 E2E-flagged store-probe tests. The probe matrix covers exact multi-source, combined good-plus-missing metadata, combined good-plus-mismatch, orphan association, duplicate output, and missing source. One earlier full run exited the Swift Testing helper with signal 11 without an assertion failure; the exact unchanged command then passed twice, so no source workaround was applied.
- The exact Apple Development-signed `<repository-root>/dist/ClipVault.app` passed launch verification, E2E smoke, strict code-signature verification, plist/privacy lint, linkage inspection, and shipping-marker absence. Fresh exact-path signed runtime PID was 2770.
- A signed disposable E2E build verified live one-source and two-source success, ordered progress, live cancel with zero saved outputs, live validator failure with zero saved outputs, Open Prompts navigation, normal generated-clip controls, original/selection preservation, and relaunch persistence. Final review closure `9abf852` replaced per-source filtering with `--verify-generated-prompt-batch`: it inventories all canonical `ClipVault AI` Prompts rows first and requires exact equality between the full association multiset and resolved stored-source IDs. The signed three-source batch passed with 3 expected tokens, 3 resolved sources, 3 generated rows, 3 associations, and zero missing/unexpected/duplicate rows; a fourth missing-source sentinel exited fail-closed with status 65. Probe output contains only counts and IDs. Shipping validation forbids both the probe command and output marker.
...
- Runtime review found a real persisted-expanded cold-launch fault: two `glassEffect() tried to update multiple times per frame` entries without AX capture. Bisection isolated launch-visible inline native-glass controls; commit `4269960` switched only that path to the existing material-backed surface while preserving 34/30-point sizing, tints, help, and AX semantics. Final review closure `9abf852` runs one policy function over the complete `AIActionPanel.swift` source and complete in-memory mutations of that exact source: native glass injected into either the shared Ask helper or inline branch is rejected, while production passes and inspector Ask retains prominent native glass. The latest exact signed cold launch, PID 2770, recorded `FOLLOWUP_GLASS_COUNT=0`.
...
#### Final Whole-Branch Review Remediation — 2026-07-13

- Commit `ed75ed0` closes the four final behavior findings with TDD evidence. Reconciliation now rejects any noncanonical use of either reserved Prompts identity before planning mutations in both stores; an exact legacy-title manual collection remains adoptable. The production first-reload seam reconciles folders before fetching clips, so migrated canonical membership is present in the first published snapshot.
- Validator preservation now covers straight, curly, and backtick literals, required JSON/YAML/CSV/Markdown formats, and context-established kebab identifiers without freezing contractions, possessives, nonrequired format mentions, or ordinary hyphenated prose. The workflow publishes an observable Saving state, suspends once through a deterministic commit boundary, then enters the synchronous atomic save without another cancellation check; Saving exposes no Cancel control.
- Focused final suites passed 30 folder/migration, 12 persistence, 52 prompt-workflow/validator, 8 workspace-presentation, and 9 E2E-flagged store-probe tests. Fresh complete gates passed 153 Swift tests in 17 suites, 4 Rust tests, strict release warnings, rustfmt, strict Clippy, shell syntax, component policies, signed rebuild/E2E, strict signature, privacy/plist/linkage, and shipping-marker checks.
- A signed disposable three-source batch passed with exact 3 expected, resolved, generated, and associated rows and identical source-ID multisets; a missing-source sentinel failed closed with status 65. A disposable persisted-expanded cold launch (PID 27140, 18:06:21-18:06:33) and the exact signed production artifact cold launch (PID 27528, 18:07:38-18:07:50) each recorded zero duplicate-glass warnings. Only the known Core Spotlight `-1000` host diagnostic appeared; production preferences and system settings were not changed.
- Final label remains **repo-ready and interaction-clean for the on-device prompt enhancer; blocked:external for complete all-controls keyboard-focus proof**. No release-candidate, upload, merge, or publication claim is made. Detailed local evidence is in `.superpowers/sdd/final-review-fixes-report.md` and `.superpowers/sdd/task-6-report.md`; the eight historical audit directories remain untouched.

#### Validator Local-Obligation Re-Review — 2026-07-13

- Focused RED at `2578478` produced 10 issues in the 56-test PromptEnhancer suite: eight dropped-format false passes across the previously unrecognized `<FORMAT> only` and `Answer in <FORMAT>` forms, plus two mixed-clause failures where clause-wide identifier context incorrectly froze `outcome-first` and `user-friendly`. Existing `must be` and `result in` cases were already caught, but remain in the full all-format obligation matrix.
- Commit `933323b` replaces clause-wide format word inference with explicit local obligation grammar for `<FORMAT> only`, `must be <FORMAT>`, `Answer/result/response/output in <FORMAT>`, bounded action-plus-`as`/`in` forms, and explicit required-format lists. Output matching remains case-insensitive. `Compare JSON parsers` and the existing JSON/YAML parser comparison remain rewritable.
- Kebab context now binds to the immediately adjacent identifier token, with optional `named`/`called`, while established shapes such as `x-*`, `*-id`, `*-version`, `*-type`, and `*-retries` remain invariant. `request-id` and `cache-control` survive mixed clauses while ordinary `outcome-first` and `user-friendly` prose can be rewritten; dropping either locally bound identifier fails closed.
- Focused GREEN passed 56/56. Fresh complete gates passed 157 Swift tests in 17 suites, 4 Rust tests, release warnings-as-errors, rustfmt, strict Clippy, shell syntax, and component policies. The exact signed production app passed rebuild/launch, E2E smoke, strict codesign, plist/privacy lint, linkage, and forbidden-marker checks.
- The rebuilt signed disposable batch again passed exact 3 expected/resolved/generated/associated rows with identical source IDs; a missing-source sentinel exited 65. The exact production cold launch, PID 35230 from 18:19:40 through 18:19:52, recorded zero duplicate-glass warnings and only the known Core Spotlight `-1000` host diagnostic. No settings or real data were changed. Readiness remains **repo-ready and interaction-clean; blocked:external only for complete all-controls keyboard-focus proof**.
