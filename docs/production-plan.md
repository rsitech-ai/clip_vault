...
- E2E database probes: every `sqlite3` process is bounded to two seconds by repo-owned shell logic; host container access timeouts exit `74` with an explicit permissions diagnostic instead of blocking the outer 20-second polling loop.
- Commands:
  - `./script/test.sh`
...
- `script/e2e_smoke.sh` routes all five SQLite probes through a repo-owned process timeout. The focused shell test covers 25 immediate successes, delayed success after shell initialization delay, independent child exits `124`/`143`, ordinary nonzero stdout/stderr, TERM-resistant timeout cleanup, sourced trap restoration, and external HUP/INT/TERM cleanup with statuses `129`/`130`/`143`.
- The live E2E still cannot read the sandbox SwiftData store from this automation host. It now fails quickly and truthfully: cold exit `74` in 17s including rebuild/signing, followed by cached exit `74` in 4s, each with `Host access timeout` after the SQLite process reaches its two-second deadline. Capture/dedupe/restart success is not claimed for this pass.
