...
- `script/e2e_smoke.sh` routes all five SQLite probes through a repo-owned process timeout. The focused shell test proves a successful fake query, exit `74` for a TERM-resistant blocking fake, child reaping, and temporary-file cleanup.
- The live E2E still cannot read the sandbox SwiftData store from this automation host. It now fails quickly and truthfully: exit `74`, `real 4.71s`, with `Host access timeout` after the SQLite process reaches its two-second deadline. Capture/dedupe/restart success is not claimed for this pass.
