...
The current source, release build, isolated runtime, E2E path, documentation links, dependency checks, and current-tree hygiene pass locally. The repository is now owned by `rsitech-ai` and remains private. The owner approved Apache-2.0, copyright ownership by Rafal Sikora, RSI Tech maintenance, `info@rsitech.ai` for public/confidential contact, and a full retained-history rewrite to the approved GitHub no-reply address. Publication remains blocked until that rewrite, exact-head hosted CI, private vulnerability reporting, and default-branch protection are verified.
...
| Public open-source repository | HOLD | Namespace, license/copyright, brand, and contacts are approved. The authorized history rewrite, visibility/security settings, and exact-head hosted CI remain to be completed. |
...
| Git-history privacy | IN PROGRESS | The owner authorized rewriting all retained commits to `24563931+s1korrrr@users.noreply.github.com`; exact-ref verification is required after force-update. |
...
Complete and verify the authorized history rewrite, rerun exact-head hosted CI, enable repository safeguards, and publish the source repository. Binary publication additionally requires a scoped notarization credential profile and clean-account runtime proof.
