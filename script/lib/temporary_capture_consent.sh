...
preseed_capture_consent_for_verify() {
  ...
  CAPTURE_CONSENT_KEY="$2"

  if CAPTURE_CONSENT_ORIGINAL_VALUE="$($CAPTURE_CONSENT_DEFAULTS_BIN read "$CAPTURE_CONSENT_BUNDLE_ID" "$CAPTURE_CONSENT_KEY" 2>/dev/null)"; then
  ...
  "$CAPTURE_CONSENT_DEFAULTS_BIN" write "$CAPTURE_CONSENT_BUNDLE_ID" "$CAPTURE_CONSENT_KEY" -bool true
}
...
restore_capture_consent_after_verify() {
  ...
  if [[ "$CAPTURE_CONSENT_HAD_VALUE" == true ]]; then
    ...
    "$CAPTURE_CONSENT_DEFAULTS_BIN" delete \
      "$CAPTURE_CONSENT_BUNDLE_ID" \
      "$CAPTURE_CONSENT_KEY" >/dev/null 2>&1 || true
  fi
